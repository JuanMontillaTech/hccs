    ...
    <nav class="navbar navbar-default">
      <div class="container-fluid">
        <div class="navbar-header">
          <div>Listado de notas débito</div>
        </div>
  ...
  methods: {
    ...
    newRecord() {
      this.$router.push({
        ...
        query: { form: "notasDeDebito" },
      });
    },
    showSchema(id) {
      this.$router.push({
        path: "/Ingresos/Facturas",
        query: { id: id, action: "show", form: "notasDeDebito" },
      });
    },
    editModalSchema(id) {
      this.$router.push({
        path: "/Ingresos/Facturas",
        query: { id: id, action: "edit", form: "notasDeDebito" },
      });
    },
    GetAllSchemaRows() {
      this.rows = [];
      this.$axios
        ...
        .then((response) => {
          const data = response.data.data.filter(
            (transaction) => transaction.transactionsType === 6
          );
          data.map((schema) => {
            let objSchema = {
              Id: schema.id,
