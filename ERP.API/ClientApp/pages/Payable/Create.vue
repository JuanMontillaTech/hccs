  ...
  methods: {
    validateAmount() {
      if (this.payable.amount <= 0) {
        this.$toast.error("El monto debe ser mayor que cero.", "Error de validación", this.izitoastConfig);
        return false;
      }
      return true;
    },
        ...
        if (this.validateAmount()) {
          if (isValid) {
            if (this.payable.id) {
              ...
              await this.$axios.post('Payable/Create', this.payable);
              this.$toast.success("Cuenta por pagar creada correctamente.", "Éxito", this.izitoastConfig);
            }
            this.GoBack();
          }
        }
      } catch (error) {
        this.$toast.error(error.message, "Error al guardar la cuenta por pagar", this.izitoastConfig);
      }
    },
    loadData() {
      ...
      if (id) {
        ...
        this.payable.id = null; // Asegurar que el ID es nulo para una nueva cuenta por pagar
      }
    },
